<template>
    <div class="p-5 border-x border-paragraph">
    ...
    <div class="flex flex-wrap px-5 justify-center mb-5">
        ...
        <div v-for="item in mealCategory" :key="item.id">
            <span class="mr-3 underline">{{ item.strCategory }}</span>
        </div>
    </div>
    <div class="flex flex-wrap px-5 justify-center mb-5">
        <h3 class="mr-3">Show this Area : </h3>
        <div v-for="item in mealArea" :key="item.id">
            <span class="mr-3 underline">{{ item.strArea }}</span>
        </div>
    </div>
    <div class="px-5 py-3 border-t border-paragraph bg-slate-200">
        <h3>Search Result</h3>
    </div>
    <div class="grid grid-cols-2 border-t border-l border-paragraph ">
        <div v-for="meal in data" :key="meal.id" class="p-4 box-border bg-slate-50 border-b border-r border-paragraph">
...
watchEffect(async () => {
    await axios.get(`https://www.themealdb.com/api/json/v1/1/search.php?s=${searchQuery.value}`).then(res => {
